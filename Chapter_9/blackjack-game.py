...
def get_total(self):
    ...
    for card in self.cards:
        if card.value == BJ_Card.ACE_VALUE:
            contains_ace = True
    # if hand contains Ace and total is low enough, treat all Ace as 11
...
# TODO
# Add some much-needed error checking to the Blackjack game. Before a new round begins, make sure that the deck
# has enough cards.
# If not, repopulate and reshuffle it.
# Find other places where you could add error checking and create the necessary safeguards.
# 2. Write a one-card version of the game War, where each player gets a single card
# and the player with the highest card wins.
# 3. Improve the Blackjack project by allowing players to bet. Keep track of each player's bankroll and remove any player
